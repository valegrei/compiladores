...
%token  PROGRAMA ID INICIO FIN NUM ASIGNAR SUMAR SI ENTONCES MAYOR SALTARIF SALTAR SINO
/*
Program MiProg;
Begin
    x:=5;
...
asig: ID {$$ = localizaSimb(lexema, ID);} ':' '=' expr {generarCodigo(ASIGNAR,$2,$5,'-');} ';' ;
...
int esPalabraReservada(char *lexema){
    ...
    else if(!strcasecmp(lexema,"else")) return SINO;
    return ID;
}
